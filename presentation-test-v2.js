...
(async () => {
  ...
  // Step 4-5: Add 3 Process Step nodes using keyboard shortcuts
  ...
  async function addNode(x, y) {
    ...
    // Look for the context menu and click the first "Process Step" option
    const menuItems = page.locator('[role="menuitem"]');
    const processStepItem = menuItems.filter({ hasText: 'Process Step' }).first();
    await processStepItem.click({ force: true });
    await page.waitForTimeout(500);
  }
  ...
  // Try to go to step 5 if it exists
  const nextButton = page.locator('button:has-text("Next")');
  if (await nextButton.isEnabled().catch(() => false)) {
    await nextButton.click();
    await page.waitForTimeout(800);
    viewportInfo = await getViewportInfo();
    console.log('Step 5 viewport:', viewportInfo);
    await page.screenshot({
      path: path.join(screenshotDir, '07b-presentation-step-5.jpg'),
      type: 'jpeg',
      quality: 90
    });
    console.log('Screenshot: 07b-presentation-step-5.jpg');
  }
  ...
  for (let i = 0; i < Math.min(allNodes.length, 3); i++) {
    ...
    const stepInfo = await page.evaluate(() => {
      ...
      return {
        stepValue: input ? input.value : 'not found',
        label: label ? label.textContent : 'not found'
      };
    });
    console.log(`Node ${i + 1} - Reveal at Step: ${stepInfo.stepValue}`);
    nodeStepInfo.push({ nodeIndex: i + 1, ...stepInfo });
  ...
  for (let step = 1; step <= 5; step++) {
    ...
    if (step < 5 && await nextBtn.isEnabled().catch(() => false)) {
      await nextBtn.click();
      await page.waitForTimeout(800);
    } else if (step < 5) {
      console.log(`Next button disabled at step ${step}, stopping`);
      break;
    }
  }
  ...
  console.log('Saved test report to test-report.json');
